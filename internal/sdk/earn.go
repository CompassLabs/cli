...
func (s *Earn) SwapQuote(ctx context.Context, request operations.V2EarnSwapQuoteRequest, opts ...operations.Option) (*operations.V2EarnSwapQuoteResponse, error) {
	...
	opURL, err := url.JoinPath(baseURL, "/v2/earn/swap_quote")
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	res := &operations.V2EarnSwapQuoteResponse{
		HTTPMeta: components.HTTPMetadata{
			Request:  req,
			Response: httpRes,
		},
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			if o.SkipDeserialization == nil || !*o.SkipDeserialization {
				...
				var out components.EarnSwapQuoteResponse
				if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
					return nil, err
				}

				res.EarnSwapQuoteResponse = &out
			}
		default:
			rawBody, err := utils.ConsumeRawBody(httpRes)
			if err != nil {

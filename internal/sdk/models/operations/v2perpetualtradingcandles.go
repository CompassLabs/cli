// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
// Generated under the AGPL-3.0-only license.
// SPDX-License-Identifier: AGPL-3.0-only

package operations

import (
	"encoding/json"
	"fmt"
	"github.com/CompassLabs/cli/internal/sdk/models/components"
	"github.com/CompassLabs/cli/internal/sdk/optionalnullable"
	"github.com/CompassLabs/cli/internal/sdk/sdkinternal/utils"
)

// CandleInterval - Candle interval: 1m, 5m, 15m, 1h, 4h, 1d, 1w
type CandleInterval string

const (
	CandleIntervalOnem     CandleInterval = "1m"
	CandleIntervalFivem    CandleInterval = "5m"
	CandleIntervalFifteenm CandleInterval = "15m"
	CandleIntervalOneh     CandleInterval = "1h"
	CandleIntervalFourh    CandleInterval = "4h"
	CandleIntervalOned     CandleInterval = "1d"
	CandleIntervalOnew     CandleInterval = "1w"
)

func (e CandleInterval) ToPointer() *CandleInterval {
	return &e
}
func (e *CandleInterval) UnmarshalJSON(data []byte) error {
	var v string
	if err := json.Unmarshal(data, &v); err != nil {
		return err
	}
	switch v {
	case "1m":
		fallthrough
	case "5m":
		fallthrough
	case "15m":
		fallthrough
	case "1h":
		fallthrough
	case "4h":
		fallthrough
	case "1d":
		fallthrough
	case "1w":
		*e = CandleInterval(v)
		return nil
	default:
		return fmt.Errorf("invalid value for CandleInterval: %v", v)
	}
}

type V2PerpetualTradingCandlesRequest struct {
	// Asset ticker (e.g. AAPL, GOLD, EUR). The xyz: HIP-3 DEX prefix is added server-side if not already present.
	Symbol string `queryParam:"style=form,explode=true,name=symbol"`
	// Candle interval: 1m, 5m, 15m, 1h, 4h, 1d, 1w
	Interval CandleInterval `queryParam:"style=form,explode=true,name=interval"`
	// Number of candles to return (max 5000, capped by Hyperliquid).
	Limit *int64 `queryParam:"style=form,explode=true,name=limit"`
	// Optional start of the candle window in unix milliseconds. If omitted, computed as end_time - limit * interval.
	StartTime optionalnullable.OptionalNullable[int64] `queryParam:"style=form,explode=true,name=start_time"`
	// Optional end of the candle window in unix milliseconds. Defaults to now.
	EndTime optionalnullable.OptionalNullable[int64] `queryParam:"style=form,explode=true,name=end_time"`
}

func (v *V2PerpetualTradingCandlesRequest) GetSymbol() string {
	if v == nil {
		return ""
	}
	return v.Symbol
}

func (v *V2PerpetualTradingCandlesRequest) GetInterval() CandleInterval {
	if v == nil {
		return CandleInterval("")
	}
	return v.Interval
}

func (v *V2PerpetualTradingCandlesRequest) GetLimit() *int64 {
	if v == nil {
		return nil
	}
	return v.Limit
}

func (v *V2PerpetualTradingCandlesRequest) GetStartTime() optionalnullable.OptionalNullable[int64] {
	if v == nil {
		return nil
	}
	return v.StartTime
}

func (v *V2PerpetualTradingCandlesRequest) GetEndTime() optionalnullable.OptionalNullable[int64] {
	if v == nil {
		return nil
	}
	return v.EndTime
}

// #region class-body-v2perpetualtradingcandlesrequest
// #endregion class-body-v2perpetualtradingcandlesrequest

type V2PerpetualTradingCandlesResponse struct {
	HTTPMeta components.HTTPMetadata `json:"-"`
	// Successful Response
	PerpetualTradingCandlesResponse *components.PerpetualTradingCandlesResponse
}

func (v V2PerpetualTradingCandlesResponse) MarshalJSON() ([]byte, error) {
	return utils.MarshalJSON(v, "", false)
}

func (v *V2PerpetualTradingCandlesResponse) UnmarshalJSON(data []byte) error {
	if err := utils.UnmarshalJSON(data, &v, "", false, nil); err != nil {
		return err
	}
	return nil
}

func (v *V2PerpetualTradingCandlesResponse) GetHTTPMeta() components.HTTPMetadata {
	if v == nil {
		return components.HTTPMetadata{}
	}
	return v.HTTPMeta
}

func (v *V2PerpetualTradingCandlesResponse) GetPerpetualTradingCandlesResponse() *components.PerpetualTradingCandlesResponse {
	if v == nil {
		return nil
	}
	return v.PerpetualTradingCandlesResponse
}

// #region class-body-v2perpetualtradingcandlesresponse
// #endregion class-body-v2perpetualtradingcandlesresponse

...
import (
	...
	"github.com/CompassLabs/cli/internal/cli/onramp"
...
func NewRootCommand() (*cobra.Command, error) {
	...
	rootCmd := &cobra.Command{
		Use:           "compass",
		Short:         "Compass API: Compass Labs DeFi API",
		Long:          "Compass API: Compass Labs DeFi API",
		SilenceUsage:  true,
		SilenceErrors: true,
		...
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := config.Init("compass", "COMPASS"); err != nil {
				return err
			}
	...
	if err := gassponsorship.InitGasSponsorshipRoot(rootCmd); err != nil {
		return nil, fmt.Errorf("init gas-sponsorship: %w", err)
	}
	if err := earn.InitEarnRoot(rootCmd); err != nil {
		return nil, fmt.Errorf("init earn: %w", err)
	}
	if err := credit.InitCreditRoot(rootCmd); err != nil {
		return nil, fmt.Errorf("init credit: %w", err)
	}
	if err := perpetualtrading.InitPerpetualTradingRoot(rootCmd); err != nil {
		return nil, fmt.Errorf("init perpetual-trading: %w", err)
	}
	if err := tokenizedassets.InitTokenizedAssetsRoot(rootCmd); err != nil {
		return nil, fmt.Errorf("init tokenized-assets: %w", err)
	}
	if err := onramp.InitOnrampRoot(rootCmd); err != nil {
		return nil, fmt.Errorf("init onramp: %w", err)
	}
	...
	rootCmd.PersistentFlags().String("api-key-auth", "", "Your Compass API Key. Get your key [here](https://www.compasslabs.ai/dashboard).")

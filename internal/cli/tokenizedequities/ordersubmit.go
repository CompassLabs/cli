...
func initOrderSubmitCmd(parent *cobra.Command) error {
	var cmd = &cobra.Command{
		...
		Example: "  compass tokenized-equities order-submit --signed-order '{\"key\":\"<value>\"}' --signature <value> --extension mp2 --quote-id <id>",
		RunE:    runOrderSubmitCmd,
		Aliases: []string{"osu"},
	}
	flagutil.RegisterFlags(cmd, orderSubmitCmdMeta)
	if err := flagutil.ValidateMeta[components.TokenizedEquitiesSubmitOrderRequest](orderSubmitCmdMeta); err != nil {
...
func runOrderSubmitCmd(cmd *cobra.Command, args []string) error {
	...
	request, err := flagutil.BuildRequest[components.TokenizedEquitiesSubmitOrderRequest](cmd, orderSubmitCmdMeta, "", "body")
	if err != nil {
		return err
	}
	...
	res, err := s.TokenizedEquities.OrderSubmit(cmd.Context(), *request, sdkOpts...)
	if err != nil {

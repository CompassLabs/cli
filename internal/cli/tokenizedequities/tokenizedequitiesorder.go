...
var tokenizedEquitiesOrderCmdMeta = []flagutil.FlagMeta{
...
// initTokenizedEquitiesOrderCmd initializes the tokenized-equities-order command.
func initTokenizedEquitiesOrderCmd(parent *cobra.Command) error {
	var cmd = &cobra.Command{
		Use:     "tokenized-equities-order",
		...
		Example: "  compass tokenized-equities tokenized-equities-order --from-token <value> --to-token <value> --amount 838.26 --owner <value>",
		RunE:    runTokenizedEquitiesOrderCmd,
		Aliases: []string{"teo"},
	}
	flagutil.RegisterFlags(cmd, tokenizedEquitiesOrderCmdMeta)
	if err := flagutil.ValidateMeta[components.TokenizedEquitiesBuildOrderRequest](tokenizedEquitiesOrderCmdMeta); err != nil {
		return fmt.Errorf("invalid metadata for tokenized-equities-order: %w", err)
	}
...
// runTokenizedEquitiesOrderCmd executes the tokenized-equities-order command.
func runTokenizedEquitiesOrderCmd(cmd *cobra.Command, args []string) error {
	if usage.UsageRequested(cmd) {
		return usage.EmitSchema(cmd, cmd.OutOrStdout())
	}
	if interactive.ShouldPrompt(cmd, tokenizedEquitiesOrderCmdMeta) {
		if err := interactive.PromptAndSetFlags(cmd, tokenizedEquitiesOrderCmdMeta); err != nil {
			return err
		}
	}
	request, err := flagutil.BuildRequest[components.TokenizedEquitiesBuildOrderRequest](cmd, tokenizedEquitiesOrderCmdMeta, "", "body")
	if err != nil {
		return err
	}
	...
	res, err := s.TokenizedEquities.TokenizedEquitiesOrder(cmd.Context(), *request, sdkOpts...)
	if err != nil {

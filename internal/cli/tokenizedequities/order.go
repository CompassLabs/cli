...
func initOrderCmd(parent *cobra.Command) error {
	var cmd = &cobra.Command{
		...
		Example: "  compass tokenized-equities order --from-token <value> --to-token <value> --amount 838.26 --owner <value>",
		RunE:    runOrderCmd,
	}
	flagutil.RegisterFlags(cmd, orderCmdMeta)
	if err := flagutil.ValidateMeta[components.TokenizedEquitiesBuildOrderRequest](orderCmdMeta); err != nil {
...
func runOrderCmd(cmd *cobra.Command, args []string) error {
	...
	request, err := flagutil.BuildRequest[components.TokenizedEquitiesBuildOrderRequest](cmd, orderCmdMeta, "", "body")
	if err != nil {
		return err
	}
	...
	res, err := s.TokenizedEquities.Order(cmd.Context(), *request, sdkOpts...)
	if err != nil {

...
var tokenizedAssetsQuoteCmdMeta = []flagutil.FlagMeta{
...
// initTokenizedAssetsQuoteCmd initializes the tokenized-assets-quote command.
func initTokenizedAssetsQuoteCmd(parent *cobra.Command) error {
	var cmd = &cobra.Command{
		Use:     "tokenized-assets-quote",
		...
		Example: "  compass tokenized-assets tokenized-assets-quote --chain base --from-token <value> --to-token <value> --amount 226.42 --owner <value>",
		RunE:    runTokenizedAssetsQuoteCmd,
		Aliases: []string{"taq"},
	}
	flagutil.RegisterFlags(cmd, tokenizedAssetsQuoteCmdMeta)
	if err := flagutil.ValidateMeta[components.TokenizedAssetsQuoteRequest](tokenizedAssetsQuoteCmdMeta); err != nil {
		return fmt.Errorf("invalid metadata for tokenized-assets-quote: %w", err)
	}
...
// runTokenizedAssetsQuoteCmd executes the tokenized-assets-quote command.
func runTokenizedAssetsQuoteCmd(cmd *cobra.Command, args []string) error {
	if usage.UsageRequested(cmd) {
		return usage.EmitSchema(cmd, cmd.OutOrStdout())
	}
	if interactive.ShouldPrompt(cmd, tokenizedAssetsQuoteCmdMeta) {
		if err := interactive.PromptAndSetFlags(cmd, tokenizedAssetsQuoteCmdMeta); err != nil {
			return err
		}
	}
	request, err := flagutil.BuildRequest[components.TokenizedAssetsQuoteRequest](cmd, tokenizedAssetsQuoteCmdMeta, "", "body")
	if err != nil {
		return err
	}
	...
	res, err := s.TokenizedAssets.TokenizedAssetsQuote(cmd.Context(), *request, sdkOpts...)
	if err != nil {

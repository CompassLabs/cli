...
var positionsAllCmdMeta = []flagutil.FlagMeta{
...
// initPositionsAllCmd initializes the positions-all command.
func initPositionsAllCmd(parent *cobra.Command) error {
	...
	if err := flagutil.ValidateMeta[operations.V2EarnPositionsAllRequest](positionsAllCmdMeta); err != nil {
		return fmt.Errorf("invalid metadata for positions-all: %w", err)
	}
	parent.AddCommand(cmd)
	return nil
}

// runPositionsAllCmd executes the positions-all command.
func runPositionsAllCmd(cmd *cobra.Command, args []string) error {
	...
	res, err := s.Earn.PositionsAll(cmd.Context(), *req, sdkOpts...)
	if err != nil {

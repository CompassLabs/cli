...
var earnManageCmdMeta = []flagutil.FlagMeta{
...
// initEarnManageCmd initializes the earn-manage command.
func initEarnManageCmd(parent *cobra.Command) error {
	var cmd = &cobra.Command{
		...
		Example: "  compass earn earn-manage --venue '{\"type\":\"VAULT\",\"vault_address\":\"0x7BfA7C4f149E7415b73bdeDfe609237e29CBF34A\"}' --action DEPOSIT --amount 0.01 --owner 0x01E62835dd7F52173546A325294762143eE4a882 --chain base",
		RunE:    runEarnManageCmd,
		Aliases: []string{"em"},
	}
	flagutil.RegisterFlags(cmd, earnManageCmdMeta)
	if err := flagutil.ValidateMeta[components.EarnManageRequest](earnManageCmdMeta); err != nil {
		return fmt.Errorf("invalid metadata for earn-manage: %w", err)
	}
...
// runEarnManageCmd executes the earn-manage command.
func runEarnManageCmd(cmd *cobra.Command, args []string) error {
	if usage.UsageRequested(cmd) {
		return usage.EmitSchema(cmd, cmd.OutOrStdout())
	}
	if interactive.ShouldPrompt(cmd, earnManageCmdMeta) {
		if err := interactive.PromptAndSetFlags(cmd, earnManageCmdMeta); err != nil {
			return err
		}
	}
	request, err := flagutil.BuildRequest[components.EarnManageRequest](cmd, earnManageCmdMeta, "", "body")
	if err != nil {
		return err
	}
	...
	res, err := s.Earn.EarnManage(cmd.Context(), *request, sdkOpts...)
	if err != nil {

...
var transferCmdMeta = []flagutil.FlagMeta{
	...
	{FlagName: "chain", Shorthand: "c", FieldPath: "Chain", Kind: flagutil.FlagKindEnum, Required: true, EnumValues: []string{"arbitrum", "base", "bsc", "ethereum", "hyperevm", "tempo"}, Description: "Blockchain network (options: arbitrum, base, bsc, ethereum, hyperevm, tempo) [required]"},
	...
	{FlagName: "spender", Shorthand: "s", FieldPath: "Spender", Kind: flagutil.FlagKindString, Optional: true, Description: "The address that will call Permit2's permitTransferFrom to execute the transfer. When `action` is 'DEPOSIT' and `gas_sponsorship` is `true`: - If provided, the signature will authorize this address (typically a gas sponsor) to pull tokens. - If not provided, defaults to the Earn Account (Safe) address, allowing the transfer to be included in a bundle transaction where the Safe pulls the tokens itself."},
	{FlagName: "recipient", Shorthand: "r", FieldPath: "Recipient", Kind: flagutil.FlagKindString, Optional: true, Description: "Optional recipient address for withdrawals. When `action` is 'WITHDRAW': - If provided, tokens will be sent to this address instead of the owner. - If not provided, defaults to the owner's address. "},
}

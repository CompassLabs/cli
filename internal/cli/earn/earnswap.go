...
var earnSwapCmdMeta = []flagutil.FlagMeta{
...
// initEarnSwapCmd initializes the earn-swap command.
func initEarnSwapCmd(parent *cobra.Command) error {
	var cmd = &cobra.Command{
		...
		Example: "  compass earn earn-swap --token-in USDC --token-out USDT --amount-in 0.01 --owner 0x01E62835dd7F52173546A325294762143eE4a882 --chain base",
		RunE:    runEarnSwapCmd,
		Aliases: []string{"es"},
	}
	flagutil.RegisterFlags(cmd, earnSwapCmdMeta)
	if err := flagutil.ValidateMeta[components.EarnSwapRequest](earnSwapCmdMeta); err != nil {
		return fmt.Errorf("invalid metadata for earn-swap: %w", err)
	}
...
// runEarnSwapCmd executes the earn-swap command.
func runEarnSwapCmd(cmd *cobra.Command, args []string) error {
	if usage.UsageRequested(cmd) {
		return usage.EmitSchema(cmd, cmd.OutOrStdout())
	}
	if interactive.ShouldPrompt(cmd, earnSwapCmdMeta) {
		if err := interactive.PromptAndSetFlags(cmd, earnSwapCmdMeta); err != nil {
			return err
		}
	}
	request, err := flagutil.BuildRequest[components.EarnSwapRequest](cmd, earnSwapCmdMeta, "", "body")
	if err != nil {
		return err
	}
	...
	res, err := s.Earn.EarnSwap(cmd.Context(), *request, sdkOpts...)
	if err != nil {

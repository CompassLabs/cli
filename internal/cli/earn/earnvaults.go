...
var earnVaultsCmdMeta = []flagutil.FlagMeta{
	...
	{FlagName: "chain", Shorthand: "c", FieldPath: "Chain", Kind: flagutil.FlagKindJSON, Optional: true, Annotations: `queryParam:"style=form,explode=true,name=chain"`, Description: "Optional chain filter. If not provided, returns vaults for all chains. (options: base, ethereum, arbitrum, hyperevm)"},
...
// initEarnVaultsCmd initializes the earn-vaults command.
func initEarnVaultsCmd(parent *cobra.Command) error {
	var cmd = &cobra.Command{
		...
		Example: "  compass earn earn-vaults --order-by tvl_usd",
		RunE:    runEarnVaultsCmd,
		Aliases: []string{"ev"},
	}
	flagutil.RegisterFlags(cmd, earnVaultsCmdMeta)
	if err := flagutil.ValidateMeta[operations.V2EarnVaultsRequest](earnVaultsCmdMeta); err != nil {
		return fmt.Errorf("invalid metadata for earn-vaults: %w", err)
	}
	parent.AddCommand(cmd)
	return nil
}

// runEarnVaultsCmd executes the earn-vaults command.
func runEarnVaultsCmd(cmd *cobra.Command, args []string) error {
	if usage.UsageRequested(cmd) {
		return usage.EmitSchema(cmd, cmd.OutOrStdout())
	}
	if interactive.ShouldPrompt(cmd, earnVaultsCmdMeta) {
		if err := interactive.PromptAndSetFlags(cmd, earnVaultsCmdMeta); err != nil {
			return err
		}
	}
	req, err := flagutil.BuildRequest[operations.V2EarnVaultsRequest](cmd, earnVaultsCmdMeta, "", "")
	if err != nil {
		return err
	}
	...
	res, err := s.Earn.EarnVaults(cmd.Context(), *req, sdkOpts...)
	if err != nil {

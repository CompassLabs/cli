...
var earnTransferCmdMeta = []flagutil.FlagMeta{
...
// initEarnTransferCmd initializes the earn-transfer command.
func initEarnTransferCmd(parent *cobra.Command) error {
	var cmd = &cobra.Command{
		...
		Example: "  compass earn earn-transfer --owner 0x01E62835dd7F52173546A325294762143eE4a882 --chain base --token USDC --amount 0.01 --action DEPOSIT",
		RunE:    runEarnTransferCmd,
		Aliases: []string{"et"},
	}
	flagutil.RegisterFlags(cmd, earnTransferCmdMeta)
	if err := flagutil.ValidateMeta[components.EarnTransferRequest](earnTransferCmdMeta); err != nil {
		return fmt.Errorf("invalid metadata for earn-transfer: %w", err)
	}
...
// runEarnTransferCmd executes the earn-transfer command.
func runEarnTransferCmd(cmd *cobra.Command, args []string) error {
	if usage.UsageRequested(cmd) {
		return usage.EmitSchema(cmd, cmd.OutOrStdout())
	}
	if interactive.ShouldPrompt(cmd, earnTransferCmdMeta) {
		if err := interactive.PromptAndSetFlags(cmd, earnTransferCmdMeta); err != nil {
			return err
		}
	}
	request, err := flagutil.BuildRequest[components.EarnTransferRequest](cmd, earnTransferCmdMeta, "", "body")
	if err != nil {
		return err
	}
	...
	res, err := s.Earn.EarnTransfer(cmd.Context(), *request, sdkOpts...)
	if err != nil {

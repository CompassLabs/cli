...
func initExecuteCmd(parent *cobra.Command) error {
	var cmd = &cobra.Command{
		...
		Example: "  compass global-markets-perps execute --action '{\"key\":\"<value>\",\"key1\":\"<value>\"}' --nonce 511479 --signature <value>",
		RunE:    runExecuteCmd,
	}
	flagutil.RegisterFlags(cmd, executeCmdMeta)
	if err := flagutil.ValidateMeta[components.GlobalMarketsPerpsExecuteRequest](executeCmdMeta); err != nil {
...
func runExecuteCmd(cmd *cobra.Command, args []string) error {
	...
	if interactive.ShouldPrompt(cmd, executeCmdMeta) {
		if err := interactive.PromptAndSetFlags(cmd, executeCmdMeta); err != nil {
			return err
		}
	}
	request, err := flagutil.BuildRequest[components.GlobalMarketsPerpsExecuteRequest](cmd, executeCmdMeta, "", "body")
	if err != nil {
		return err
	}
	...
	res, err := s.GlobalMarketsPerps.Execute(cmd.Context(), *request, sdkOpts...)
	if err != nil {

...
// initGasSponsorshipPrepareCmd initializes the gas-sponsorship-prepare command.
func initGasSponsorshipPrepareCmd(parent *cobra.Command) error {
	var cmd = &cobra.Command{
		Use:     "gas-sponsorship-prepare",
...
// runGasSponsorshipPrepareCmd executes the gas-sponsorship-prepare command.
func runGasSponsorshipPrepareCmd(cmd *cobra.Command, args []string) error {
	if usage.UsageRequested(cmd) {
		return usage.EmitSchema(cmd, cmd.OutOrStdout())
	}
	if interactive.ShouldPrompt(cmd, gasSponsorshipPrepareCmdMeta) {
		if err := interactive.PromptAndSetFlags(cmd, gasSponsorshipPrepareCmdMeta); err != nil {
			return err
		}
	}
	request, err := flagutil.BuildRequest[components.SponsorGasRequest](cmd, gasSponsorshipPrepareCmdMeta, "", "body")
	if err != nil {
		return err
	}
	...
	res, err := s.GasSponsorship.GasSponsorshipPrepare(cmd.Context(), *request, sdkOpts...)
	if err != nil {

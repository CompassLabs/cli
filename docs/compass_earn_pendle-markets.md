...
```
  -c, --chain string               Optional chain filter. If not provided, returns markets for all chains. (options: base, ethereum, arbitrum, hyperevm, tempo, bsc, robinhood, ethereum_sepolia)
...
* `compass earn pendle-markets --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `compass earn pendle-markets --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)

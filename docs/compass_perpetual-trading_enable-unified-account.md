## compass perpetual-trading enable-unified-account
...
```
compass perpetual-trading enable-unified-account [flags]
```

### Examples

```
  compass perpetual-trading enable-unified-account --owner <value>
```
...
```
  ...
  -h, --help           help for enable-unified-account
...
* `compass perpetual-trading enable-unified-account --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `compass perpetual-trading enable-unified-account --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `compass perpetual-trading enable-unified-account --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)

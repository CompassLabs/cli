...
```
  ...
  -c, --chain string    The chain to use. (options: base, ethereum, arbitrum, hyperevm, tempo, bsc, robinhood, ethereum_sepolia)
...
* `compass tokenized-assets create-account --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `compass tokenized-assets create-account --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `compass tokenized-assets create-account --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)

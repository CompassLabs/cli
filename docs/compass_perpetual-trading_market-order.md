## compass perpetual-trading market-order
...
```
compass perpetual-trading market-order [flags]
```

### Examples

```
  compass perpetual-trading market-order --owner 0x06A9aF046187895AcFc7258450B15397CAc67400 --asset AAPL --side buy --size 10.0
```
...
```
  ...
  -r, --reduce-only              If true, order can only reduce an existing position
      ...
      --slippage-percent float   Max slippage percentage for price protection
```
...
* `compass perpetual-trading market-order --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `compass perpetual-trading market-order --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `compass perpetual-trading market-order --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)

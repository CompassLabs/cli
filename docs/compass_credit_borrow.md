...
```
compass credit borrow [flags]
```

### Examples

```
  compass credit borrow --owner 0xfeEb60eBf707DAA8248277058eb8D28EaeCF5425 --chain base --borrow-token WETH --borrow-amount 0.01
```
...
* `compass credit borrow --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `compass credit borrow --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `compass credit borrow --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)

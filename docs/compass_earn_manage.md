...
```
  ...
  -v, --venue string                                 JSON value (variants: AAVE: { "token": string }, PENDLE_PT: { "market_address": string, ... }, VAULT: { "vault_address": string })
...
* `compass earn manage --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `compass earn manage --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `compass earn manage --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)

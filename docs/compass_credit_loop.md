...
* `compass credit loop --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `compass credit loop --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `compass credit loop --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)

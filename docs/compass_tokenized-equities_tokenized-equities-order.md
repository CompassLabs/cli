## compass tokenized-equities tokenized-equities-order
...
```
compass tokenized-equities tokenized-equities-order [flags]
```

### Examples

```
  compass tokenized-equities tokenized-equities-order --from-token <value> --to-token <value> --amount 838.26 --owner <value>
```
...
```
  ...
  -h, --help                  help for tokenized-equities-order
      --owner string          Wallet that owns the Tokenized Equities Account. The product account address is derived deterministically from this owner. The owner signs the EIP-712 payloads returned by this endpoint (the optional approval and the order itself). [required]

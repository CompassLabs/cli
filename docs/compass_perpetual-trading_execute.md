...
* `compass perpetual-trading execute --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `compass perpetual-trading execute --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `compass perpetual-trading execute --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)

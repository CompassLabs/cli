...
```
compass credit positions [flags]
```

### Examples

```
  compass credit positions --chain base --owner 0x01E62835dd7F52173546A325294762143eE4a882
```
...
* `compass credit positions --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `compass credit positions --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)

## compass perpetual-trading withdraw
...
```
compass perpetual-trading withdraw [flags]
```

### Examples

```
  compass perpetual-trading withdraw --owner 0x06A9aF046187895AcFc7258450B15397CAc67400 --amount 100.0
```
...
```
  ...
  -h, --help                 help for withdraw
      --owner string         The user's EOA address (owner of the perpetual trading account) [required]
...
* `compass perpetual-trading withdraw --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `compass perpetual-trading withdraw --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `compass perpetual-trading withdraw --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)

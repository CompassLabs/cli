...
* `compass credit unloop --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `compass credit unloop --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `compass credit unloop --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)

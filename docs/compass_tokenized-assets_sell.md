...
```
  compass tokenized-assets sell --token-in <value> --token-out <value> --amount-in 8512.47 --owner <value> --chain hyperevm
```
...
* `compass tokenized-assets sell --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `compass tokenized-assets sell --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `compass tokenized-assets sell --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)

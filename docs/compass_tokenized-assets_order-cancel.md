...
* `compass tokenized-assets order-cancel --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `compass tokenized-assets order-cancel --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `compass tokenized-assets order-cancel --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)

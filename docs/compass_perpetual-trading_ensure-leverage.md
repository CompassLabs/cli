## compass perpetual-trading ensure-leverage
...
```
compass perpetual-trading ensure-leverage [flags]
```

### Examples

```
  compass perpetual-trading ensure-leverage --owner <value> --asset <value>
```
...
```
  ...
  -h, --help           help for ensure-leverage
...
* `compass perpetual-trading ensure-leverage --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `compass perpetual-trading ensure-leverage --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `compass perpetual-trading ensure-leverage --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)

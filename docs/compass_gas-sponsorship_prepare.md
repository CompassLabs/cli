...
* `compass gas-sponsorship prepare --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `compass gas-sponsorship prepare --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `compass gas-sponsorship prepare --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)

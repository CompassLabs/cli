## compass perpetual-trading cancel-order
...
```
compass perpetual-trading cancel-order [flags]
```

### Examples

```
  compass perpetual-trading cancel-order --owner 0x06A9aF046187895AcFc7258450B15397CAc67400 --asset AAPL
```
...
* `compass perpetual-trading cancel-order --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `compass perpetual-trading cancel-order --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `compass perpetual-trading cancel-order --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
